#!/bin/bash
generate_changelog() {
  # generate_changelog_from_commits $MAIN_BRANCH $DEVELOP_BRANCH $CURRENT_VERSION
...
generate_changelog_from_commits() {
	...
	# Definir si se incluirá el body
	local include_body=${CHANGELOG_INCLUDE_BODY:-false}

	# Usar delimitador personalizado para procesar título + cuerpo
	...
	while IFS= read -r line; do
		if [[ "$line" == "==END==" ]]; then
			...
			if [[ "$include_body" == "true" && -n "$current_body" ]]; then
				# Indentar el body para Markdown
