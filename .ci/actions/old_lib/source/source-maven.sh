...
lint() {
	...
	append_profile "pom.xml" ".ci/actions/conf/maven/pmd-profile.xml" $TEMP
	if log; then
...
verify() {
	...
	append_profile "pom.xml" ".ci/actions/conf/maven/jacoco-profile.xml" $TEMP
...
test() {
	...
	append_profile "pom.xml" ".ci/actions/conf/maven/pit-profile.xml" $TEMP
	if [[ "$REQUIRED_COVERAGE" != "" ]]; then
		echo "- Coverage of $REQUIRED_COVERAGE"
