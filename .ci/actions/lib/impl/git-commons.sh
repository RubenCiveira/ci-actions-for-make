...
get_existents_rc() {
	...
	git log --pretty=format:"%s" | grep "chore(release): " | grep -o "${VERSION_ESCAPED}-RC-[0-9]\+" |  sed -E "s/${VERSION_ESCAPED}-RC-([0-9]+)/\1/" |  sort -nr | head -n1
}

get_existents_snapshots() {
	local VERSION="$1"
	local VERSION_ESCAPED=$(echo "$VERSION" | sed 's/\./\\./g')
    git log --pretty=format:"%s" | grep "chore(release): " | grep -o "${VERSION_ESCAPED}-p[0-9]\+-SNAPSHOT" | sed -E "s/${VERSION_ESCAPED}-p([0-9]+)-SNAPSHOT/\1/" | sort -nr | head -n1
}
...
next_version() {
	...
	if [[ "$FINAL" == "snapshot" ]]; then
		local TARGET_REV="${MAYOR}.${MINOR}.$((PATCH+1))"
		local SNAP_NUM=$(get_existents_snapshots ${TARGET_REV})
		echo "${TARGET_REV}-p$((SNAP_NUM+1))-SNAPSHOT"
	else 
		local TARGET_REV=""
		if [[ "$MAYORS" != "" ]]; then
			local NEW_MAJOR=$((MAYOR + 1))
			TARGET_REV="$NEW_MAJOR.0.0"
		elif [[ "$MINORS" != "" ]]; then
			local NEW_MINOR=$((MINOR + 1))
			TARGET_REV="$MAYOR.$NEW_MINOR.0"
		elif [[ "$PATCHES" != "" ]]; then
			local NEW_PATCH=$((PATCH + 1))
			TARGET_REV="$MAYOR.$MINOR.$NEW_PATCH"
		else
		...
		if [[ "$FINAL" == "release" ]]; then
			local RC_NUM=$(get_existents_rc $TARGET_REV)
			TARGET_REV="${TARGET_REV}-RC-$(($RC_NUM+1))"
		fi
		echo ${TARGET_REV}
	fi
}
...
host=$REPO_HOST" | git credential fill)
	...
	local USERNAME=$(_priv_read_from_properties "username" "$CREDENTIALS")
	local PASSWORD=$(_priv_read_from_properties "password" "$CREDENTIALS")
...
create_squash_merge() {
	...
	local KIND=$(_priv_read_from_properties "kind" "$METADATA") 
	local TITLE=$(_priv_read_from_properties "title" "$METADATA")
	local ISSUE=$(_priv_read_from_properties "issue" "$METADATA")

	local MESSAGE="Development"
	if [[ "$ISSUE" != "" ]]; then
		local INFO=$(get_issue "$ISSUE")
		MESSAGE=$(_priv_read_from_properties description "$INFO")
...
lookup_pr() {
	...
	local PRIVATE_TOKEN=$(_priv_read_from_properties "private-token" "$INFO")
	local PROJECT_API_URL=$(_priv_read_from_properties "project-api-url" "$INFO")
	local REPOSITORY_KIND=$(_priv_read_from_properties "repository-kind" "$INFO")
...
check_sync_branchs() {
	printf -- "- Checking if branches are sync:"
	local first=1
