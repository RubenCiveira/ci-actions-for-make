...
run_lookup_pr_gitlab() {
	...
	echo "curl -s --header \"PRIVATE-TOKEN: $PRIVATE_TOKEN\" \"$PROJECT_API_URL/merge_requests?source_branch=$HEAD&target_branch=$BASE&state=opened\""
...
run_merge_pr_gitlab() {
	...
	if [[ "$PR" == "" ]]; then
		...
		local MR=$(_priv_read_from_properties "id" "$PR")
		local HAS_CONFLICT=$(_priv_read_from_properties "has-conflict" "$PR")
		local MERGE_STATUS=$(_priv_read_from_properties "merge-status" "$PR")
		local SQUASH=$(_priv_read_from_properties "squash" "$PR")
		local DELETE=$(_priv_read_from_properties "delete" "$PR")
		local TITLE=$(_priv_read_from_properties "title" "$PR")
		local DESCRIPTION=$(_priv_read_from_properties "description" "$PR")
		local WEB_URL=$(_priv_read_from_properties "web-url" "$PR")
		...
		if [[ "$HAS_CONFLICT" == "true" ]]; then
			echo "The MR #$MR has conflicts and cant be automaticly merged"
			exit 0
		fi
		
		if [[ "$MERGE_STATUS" == "unchecked" ]]; then
			echo "The MR #$MR need to be checked at $WEB_URL"
		fi
	
		if [[ "$MERGE_STATUS" == "checking" || "$MERGE_STATUS" == "cannot_be_merged_recheck" || "$MERGE_STATUS" == "preparing merged" ]]; then
			echo "The MR #$MR is beeng checking by the system"
			sleep 10
			run_merge_pr_gitlab "$BASE" "$HEAD" "$PROJECT_API_URL" "$REPOSITORY_KIND"
		fi
		
		if [[ "$MERGE_STATUS" == "merged" ]]; then
			echo "The MR #$MR cant be automaticly merged for $MERGE_STATUS"
			echo $PR
			exit 0		
		fi
